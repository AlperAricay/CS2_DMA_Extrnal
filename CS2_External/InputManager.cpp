...
bool c_keys::InitKeyboard()
{
	...
	this->win_logon_pid = ProcessMgr.GetPidFromName("winlogon.exe");
	if (Winver > 22000)
	{
		auto pids = ProcessMgr.GetPidListFromName("csrss.exe");
		for (size_t i = 0; i < pids.size(); i++)
		{
			auto pid = pids[i];
			uintptr_t tmp = VMMDLL_ProcessGetModuleBaseU(ProcessMgr.HANDLE, pid, (LPSTR)"win32ksgd.sys");
			uintptr_t g_session_global_slots = tmp + 0x3110;
			uintptr_t user_session_state = ProcessMgr.ReadMemory(ProcessMgr.ReadMemory(ProcessMgr.ReadMemory(g_session_global_slots, pid), pid), pid);
	...
	else
	{
		...
		bool result = VMMDLL_Map_GetEATU(ProcessMgr.HANDLE, ProcessMgr.GetPidFromName("winlogon.exe") | VMMDLL_PID_PROCESS_WITH_KERNELMEMORY, (LPSTR)"win32kbase.sys", &eat_map);
		if (!result)
			return false;
...
void c_keys::UpdateKeys()
{
	...
	VMMDLL_MemReadEx(ProcessMgr.HANDLE, this->win_logon_pid | VMMDLL_PID_PROCESS_WITH_KERNELMEMORY, gafAsyncKeyStateExport, (PBYTE)&state_bitmap, 64, NULL, VMMDLL_FLAG_NOCACHE);
